...
def line_fit(binary_warped):
	...
	nonzero = binary_warped.nonzero()		#[y, x]
	nonzeroy = np.array(nonzero[0])			#y indices
	nonzerox = np.array(nonzero[1])			#x indices
	...
	for window in range(nwindows):
		...
		##TO DO
		win_widthleft_low = leftx_current - margin
		win_widthleft_high = leftx_current + margin
		win_widthright_low = rightx_current - margin
		win_widthright_high = rightx_current + margin
		win_height_low = binary_warped.shape[0] - (window + 1) * window_height
		win_height_high = binary_warped.shape[0] - window * window_height
		
		####
		# Draw the windows on the visualization image using cv2.rectangle()
		##TO DO
		cv2.rectangle(out_img,(win_widthleft_low,win_height_low),(win_widthleft_high,win_height_high),(0,255,0), 2)
		cv2.rectangle(out_img,(win_widthright_low,win_height_low),(win_widthright_high,win_height_high),(0,255,0), 2)
		####
		# Identify the nonzero pixels in x and y within the window
		##TO DO
		left_index_inwindow = ((nonzeroy >= win_height_low) & (nonzeroy < win_height_high) & 
                          (nonzerox >= win_widthleft_low) & (nonzerox < win_widthleft_high)).nonzero()[0]
		
		right_index_inwindow = ((nonzeroy >= win_height_low) & (nonzeroy < win_height_high) & 
                           (nonzerox >= win_widthright_low) & (nonzerox < win_widthright_high)).nonzero()[0]
		####
		# Append these indices to the lists
		##TO DO
		left_lane_inds.append(left_index_inwindow)
		right_lane_inds.append(right_index_inwindow)
		####
