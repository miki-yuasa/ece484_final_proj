...
class lanenet_detector():
    ...
    def combinedBinaryImage(self, img):
        ...
        binaryImage = np.zeros_like(color_output)
    ...
    def perspective_transform(self, img, verbose=False):
        ...
        # pts2 = np.float32([[100, 390], [100, 360], [560, 390], [560, 360]])
        ...
        if self.zed == True:
            src = np.array([[int(Img_width*3.5/30), int(Img_height*30/30)],      # bottom point on left lane
                        [int(Img_width*12.5/30), int(Img_height*15.6/30)],           # top point on left lane
                        [int(Img_width*24.3/30), int(Img_height*30/30)],   # bottom point on right lane
                        [int(Img_width*16.5/30), int(Img_height*15.6/30)]], np.float32)        # top point on right lane
        else:
            src = np.array([[int(Img_width*4.5/30), int(Img_height*29/30)],      # bottom point on left lane
                            [int(Img_width*14.5/30), int(Img_height*16/30)],           # top point on left lane
                            [int(Img_width*25/30), int(Img_height*39/30)],   # bottom point on right lane
                            [int(Img_width*17/30), int(Img_height*16/30)]], np.float32)        # top point on right lane
        dst = np.array([[0, Img_height-1], 
                        [0, 0],
                        [Img_width-1, Img_height-1],
                        [Img_width-1, 0]], np.float32)
